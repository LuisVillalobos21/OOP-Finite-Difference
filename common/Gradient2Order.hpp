...
{
    std::vector<double> gradient_vector_x;
    std::vector<double> gradient_vector_y;

    Gradient2Order(Grid& grid);

    std::pair<std::vector<double> , std::vector<double>> calculateOperator(Grid& grid, Connectivity& connect, std::vector<BoundaryCondition>& BC_vector, std::vector<double>& function_values);
